    ...
    static class CPULinuxLoadValue
    {
        ...
        private static double QUERY_CPULOAD(bool a = true)
        {
            lock (syncobj)
            {
                ...
                try
                {
                    long od = (previous_cpu_occupy.user + previous_cpu_occupy.nice + previous_cpu_occupy.system + previous_cpu_occupy.idle + previous_cpu_occupy.lowait + previous_cpu_occupy.irq + previous_cpu_occupy.softirq);//first time of priority (user) Assign to od
                    long nd = (current_cpu_occupy.user + current_cpu_occupy.nice + current_cpu_occupy.system + current_cpu_occupy.idle + current_cpu_occupy.lowait + current_cpu_occupy.irq + current_cpu_occupy.softirq);
        ...
        public static bool GlobalMemoryStatus(ref MEMORY_INFO mi)
        {
